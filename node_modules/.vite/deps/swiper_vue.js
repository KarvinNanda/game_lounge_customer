import { Bt as onUpdated, Ft as onMounted, Kn as ref, Mt as onBeforeUpdate, Ot as nextTick, U as computed, Ut as provide, gn as watch, jt as onBeforeUnmount, pt as h, xt as inject } from "./vue.runtime.esm-bundler-dM5W6Okq.js";
import { C as setCSSProperty, D as getWindow, E as getDocument, S as now, T as showWarning, a as elementChildren, c as elementNextAll, d as elementParents, f as elementPrevAll, h as extend$1, i as deleteProps, o as elementIndex, p as elementStyle, r as createElement, s as elementIsChildOf, t as animateCSSModeScroll, u as elementOuterSize, v as getTranslate, w as setInnerHTML, x as nextTick$1 } from "./utils-iQnhwPxP.js";
...
function getChildren(originalSlots = {}, slidesRef, oldSlidesRef) {
	...
	const getSlidesFromElements = (els, slotName) => {
		...
		els.forEach((vnode) => {
			...
			if (isFragment && vnode.children) getSlidesFromElements(vnode.children, slotName);
			else if (vnode.type && (vnode.type.name === "SwiperSlide" || vnode.type.name === "AsyncComponentWrapper") || vnode.componentOptions && vnode.componentOptions.tag === "SwiperSlide") slides.push(vnode);
			else if (slots[slotName]) slots[slotName].push(vnode);
		});
	};
	Object.keys(originalSlots).forEach((slotName) => {
		if (typeof originalSlots[slotName] !== "function") return;
		getSlidesFromElements(originalSlots[slotName](), slotName);
	});
...
function renderVirtual(swiperRef, slides, virtualData) {
	...
	const style = swiperRef.value.isHorizontal() ? { [swiperRef.value.rtlTranslate ? "right" : "left"]: `${virtualData.offset}px` } : { top: `${virtualData.offset}px` };
	const { from, to } = virtualData;
	const loopFrom = swiperRef.value.params.loop ? -slides.length : 0;
	const loopTo = swiperRef.value.params.loop ? slides.length * 2 : slides.length;
	const slidesToRender = [];
	for (let i = loopFrom; i < loopTo; i += 1) if (i >= from && i <= to && slidesToRender.length < slides.length) slidesToRender.push(slides[getSlideIndex(i)]);
	return slidesToRender.map((slide) => {
		if (!slide.props) slide.props = {};
		if (!slide.props.style) slide.props.style = {};
		slide.props.swiperRef = swiperRef;
		slide.props.style = style;
		if (slide.type) return h(slide.type, { ...slide.props }, slide.children);
		else if (slide.componentOptions) return h(slide.componentOptions.Ctor, { ...slide.props }, slide.componentOptions.children);
	});
}
var Swiper = {
	name: "Swiper",
